        ...
        <div class="container mt-5">
                ...
                <?php
                    if (isset($_POST['sign_up'])) {
                        ...
                        $check = "SELECT*FROM signup_from WHERE phone_number = '$phone_number'";
                        $phone_check = mysqli_query($link,$check);
                        while ($row_check = mysqli_fetch_assoc($phone_check)) {
                            $find_phone_number = $row_check['phone_number'];
                        }
                        if ($find_phone_number !="") {
                            echo "<script>alert('$phone_number Already Registered')</script>";
                        }
                        else {
                            $insert_into = "INSERT INTO signup_form(first_name,last_name,email,phone_number,password,confirm_password) VALUE('$first_name','$last_name','$email','$phone_number','$password','$confirm_password')";
                            $insert = mysqli_query($link,$insert_into);
                            if ($insert == true) {
                                echo "<script>alert('Thanks For Registering With Us')</script>";
                            }
                            else {
                                echo"<script>alert('An Error Occured During Registration Please Try Again Later')</script>";
                            }
                        }
                    }
                ?>
                ...
                <form action="sign_up.php" method="POST" class="form-controls shadow signup_form form2 col-md-10">
                    ...
                    <div class="row mx-auto">
                        ...
                        <input type="password" name="" class="form-control col-md-9 ml-4 mr-3 mt-3" name="password" placeholder="Enter Password">
                        <br><br>
                        <label for="password" class="form-label ml-4 mt-3">Confirm Password:</label>
                        <input type="password" name="" class="form-control col-md-9 ml-4 mt-3" name="confirm_password" placeholder="Re-enter Password">
                    </div>
